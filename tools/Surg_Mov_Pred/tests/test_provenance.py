...
def test_every_argument_of_run_is_documented_for_the_client():
    ...
    for name, spec in inspect.signature(run).parameters.items():
        if spec.kind is inspect.Parameter.KEYWORD_ONLY:
            # Injected by the server and never published, so there is no client
            # to describe it to -- `describe.py` excludes it for the same
            # reason. `data_root` is the one here.
            continue
...
def test_the_public_surface_is_run_alone():
    ...
    # Private names do not count: `_own_models` exists because `model` became
    # optional -- a caller that names no bundle gets this tool's own, found
    # under the data root the server injects.
